...
function Ideas() {
    ...
    const makeResultsAppear = async (event) => {
        ...
        try {
            const response = await fetch(`http://localhost:3001/activity/${activity}`);
    ...
    const getFinalDate = async (event, activity, game, food) => {
        ...
        try {
            const response = await fetch(`http://localhost:3001/activity/description?name=${(finalActivity)}`);
